...
export default function SessionPage() {
  ...
  async function load() {
    ...
    setBankTx(tx || [])
    const res2 = await fetch(`/api/journal/lines?organizationId=${(window as any).ORG_ID || ''}`)
    const jl = await res2.json()
    setJournalLines(jl || [])
  }

  async function match(txId: string, lineId: string) {
    const res = await fetch('/api/banking/reconcile/match', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: id, bankTransactionId: txId, journalLineId: lineId }) })
    if (res.ok) { setStatus('Matched'); load() }
    else setStatus('Match failed')
  }

  async function autoMatch() {
    setStatus('Running auto-match...')
    // simple heuristic: exact amount match
    ...
    for (const tx of bankTx) {
      ...
      if (candidates.length === 1) {
        promises.push(fetch('/api/banking/reconcile/match', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: id, bankTransactionId: tx.id, journalLineId: candidates[0].id }) }))
      }
    }
  ...
  async function getSuggestions() {
    ...
    if (!res.ok) { setStatus('Failed to fetch suggestions'); return }
    const json = await res.json()
    const map = json.suggestions || {}
    // attach suggestions to bankTx
  ...
  async function applyMappings(mappings: any[]) {
    ...
    const res = await fetch('/api/banking/reconcile/apply-batch', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: id, mappings }) })
    if (res.ok) {
  ...
  async function applyAllSuggestions() {
    ...
    const res = await fetch('/api/banking/reconcile/apply-batch', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: id, strategy: 'auto' }) })
    if (res.ok) {
      const j = await res.json()
  ...
  async function finalize() {
    const res = await fetch('/api/banking/reconcile/finalize', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: id }) })
        ...
        {bankTx.map(tx => (
          <li key={tx.id} style={{ marginBottom: 12 }}>
            ...
            <div style={{ marginTop: 6 }}>
              ...
              <button style={{ marginLeft: 8 }} onClick={() => { const sel = (document.getElementById(`sel-${tx.id}`) as HTMLSelectElement).value; if (sel) match(tx.id, sel) }}>Match</button>
