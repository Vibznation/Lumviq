...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    await prisma.$transaction(async (tx: any) => {
      ...
      await tx.auditEvent.create({ data: { organizationId: session.organizationId, actorId: user.id, action: 'bank.reconciliation.match', resourceType: 'reconciliation_item', resourceId: item.id, newState: { bankTransactionId, journalLineId, sessionId } } })
    })
  } catch (e) {
