...
/** Accounts-receivable aging: outstanding invoice balances bucketed by days past due. Accepts an optional `asOfDate` (ISO date string) to compute aging as of a past date instead of now. */
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const invoices = await prisma.invoice.findMany({
    where: { organizationId, voidedAt: null, status: { in: ['sent', 'partially_paid', 'overdue'] }, issueDate: { lte: asOfDate } },
