...
import { syncTaxLiabilities } from '../../../lib/payroll-tax'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    const filings = await prisma.taxFiling.findMany({ where: { organizationId, amount: { not: null } }, orderBy: { dueDate: 'asc' } })
  ...
  if (req.method === 'POST') {
    ...
    try {
      const filings = await prisma.$transaction((tx) => syncTaxLiabilities(tx, organizationId))
      return res.status(200).json(filings)
    } catch (err: any) {
