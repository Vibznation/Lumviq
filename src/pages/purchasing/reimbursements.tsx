...
type Reimbursement = { id: string; payeeName: string; amount: string; description: string | null; status: string }
...
function ReimbursementsContent() {
  ...
  const [form, setForm] = useState({ payeeName: '', amount: '', description: '', expenseAccountId: '' })
  ...
  async function load() {
    ...
    try {
      const [rRes, aRes] = await Promise.all([
        fetch(`/api/reimbursements?organizationId=${currentOrg.id}`, { headers: authHeaders(token) }),
        fetch(`/api/accounts?organizationId=${currentOrg.id}`, { headers: authHeaders(token) }),
      ])
      if (!rRes.ok) throw new Error('Could not load reimbursements')
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(j.error || 'Could not create reimbursement')
      }
      setForm({ payeeName: '', amount: '', description: '', expenseAccountId: '' })
      setShowForm(false)
      await load()
    } catch (err: any) {
      setError(err.message)
  ...
  async function decide(id: string, decision: 'approve' | 'reject' | 'pay') {
    setBusy(id)
    setError(null)
    try {
      const bankAccount = accounts.find((a) => a.subtype === 'bank')
      const res = await fetch(`/api/reimbursements/${id}/decide`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders(token) },
  ...
  return (
    <div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="mb-6 bg-white dark:bg-midnight-900 border border-gray-200 dark:border-midnight-800 rounded-lg p-4 grid gap-3 max-w-md">
          <div>
            ...
            <input required value={form.payeeName} onChange={(e) => setForm((f) => ({ ...f, payeeName: e.target.value }))} className="mt-1 w-full rounded-md border border-gray-300 dark:border-midnight-700 bg-white dark:bg-midnight-800 dark:text-gray-100 px-2 py-1.5 text-sm" />
          </div>
          <div>
      ...
      {loading ? (
        ...
        <p className="text-sm text-gray-500">No reimbursement requests yet.</p>
      ) : (
              ...
              {items.map((r) => (
                <tr key={r.id} className="border-t border-gray-100 dark:border-midnight-800">
                  ...
                  <td className="px-4 py-2 text-right space-x-2">
                    {r.status === 'pending' && (
                      <>
                        <button onClick={() => decide(r.id, 'approve')} disabled={busy === r.id} className="text-xs text-teal-700 dark:text-teal-400 hover:underline disabled:opacity-50">Approve</button>
                        <button onClick={() => decide(r.id, 'reject')} disabled={busy === r.id} className="text-xs text-red-600 hover:underline disabled:opacity-50">Reject</button>
                      </>
                    )}
                    {r.status === 'approved' && (
                      <button onClick={() => decide(r.id, 'pay')} disabled={busy === r.id} className="text-xs text-teal-700 dark:text-teal-400 hover:underline disabled:opacity-50">Mark paid</button>
                    )}
                  </td>
                </tr>
              ))}
